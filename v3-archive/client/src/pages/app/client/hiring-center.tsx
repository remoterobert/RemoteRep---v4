...
import ApplicationTalentDetail from 'components/talents/applicationTalentDetail';
import { useNotification } from 'contexts/NotificationContext';
import { getMatchScore } from 'services/utils/getMatchScore';
import CircleProgress from 'components/cirlceProgress';
import RatingIcon from 'components/talents/ratingIcon';
...
const ApplicationManagement: React.FC<{
    applications: TalentApplication[] | Talent[];
    ...
    useEffect(() => setSelectedApplication(null), [selectedStatus]);

    useEffect(() => {
        setPreferences(
            {
                experience: listing?.requirements,
                goals: {
    ...
    const handleUpdateApplicationStatus = ({
        ...
        if (listing) {
            updateApplication(
                listing.id,
                {
                    talentId: selectedApplication.talent.id,
    ...
    return (
        <>
            <div className="grid h-full md:grid-cols-4 shadow-md grid-cols-1 gap-[2px] shadow-md bg-background dark:bg-darkBackground">
                <div className="h-full w-full bg-background dark:bg-darkBackground overflow-x-hidden overflow-y-auto scrollbar-thin pl-8 pr-8 pb-8">
                    {applications?.length ? (
                        applications
                            .sort(
                                (a, b) =>
                                    (b?.applicationRating || 0) -
                                    (a?.applicationRating || 0)
                            )
                            .map((t: TalentApplication | Talent, i: number) => (
                                <div
                                    key={i}
                                    onClick={() => {
                                        setSelectedApplication(t);
                                        if (
                                            window.innerHeight >
                                            window.innerWidth
                                        )
                                    ...
                                    className="bg-white dark:bg-darkForeground rounded-2xl w-full border-b-[2px] border-gray-900/5 hover:shadow-xl"
                                >
                                    <div className="flex-col justify-center items-center rounded-2xl">
                                        <div className="px-4 py-4 flex justify-between items-center">
                                            <div className="inline-flex">
                                                {(t?.talent || t)?.deleted ||
                                                (t?.talent || t)?.suspended ? (
                                                    <div className="h-8 w-8 md:h-16 md:w-16 rounded-full shrink-0 bg-gray-500" />
                                                ) : (t?.talent || t)?.talentData
                                                    ?.profile?.photoUrl ? (
                                                    <img
                                                        src={
                                                            (t?.talent || t)
                                                    ...
                                                    <UserIcon className="h-12 w-12 rounded-full shrink-0 text-gray-300 bg-gray-100" />
                                                )}

                                                <div className="ml-4 font-medium text-black dark:text-white flex items-center">
                                                    <div>
                                                        <span className="text-[16px] font-bold whitespace-nowrap inline-flex">
                                                            {(t?.talent || t)
                                                                ?.deleted ? (
                                                                'Deleted User'
                                                            ) : (t?.talent || t)
                                            ...
                                            <div className="flex justify-center items-center">
                                                    ...
                                                    {bookmarks.includes(
                                                                ...
                                                                <div className="h-[32px] w-[32px] bg-background rounded-full mr-2 flex justify-center items-center">
                                                                    <SolidBookmarkIcon className="my-auto h-4 w-4 md:h-6 md:w-6 text-primary" />
                                                                </div>
                                                            )}
                                                {t?.applicationStatus &&
                                                t?.applicationRating ? (
                                    ...
                                    {t?.applicationMessage && (
                                        <div className="mb-2 w-full gap-2 px-4">
                                            <p className="text-gray-900 text-md font-medium break-all">
                                                Message from applicant:
                                            </p>
                                            <p className="mt-2 text-gray-800 text-sm max-h-24 break-all">
                                                {t?.applicationMessage?.length >
                                                160
                                                    ? `${t?.applicationMessage?.substring(
                                                          0,
                                                          160
                                                      )}...`
                                                    : t?.applicationMessage}
                        ...
                        <div className="h-[42rem] w-full flex items-center justify-center text-black dark:text-white">
                            {loading ? (
                ...
                <div className="bg-background dark:bg-darkBackground md:col-span-3 w-full h-full overflow-x-hidden overflow-y-auto scrollbar-thin hidden md:block pr-8 pb-8">
                    <div className="min-h-[550px] h-full w-full md:col-span-2 overflow-x-hidden overflow-y-auto scrollbar-thin hidden md:block rounded-2xl bg-white dark:bg-darkForeground">
                        {listing && (
                            <ApplicationTalentDetail
                                ...
                                setUpdate={setUpdate}
                                onChangeListing={handleUpdateApplicationStatus}
                                hasAccess={hasAccess}
        ...
        <>
            ...
            {!loading && (
                <>
                    ...
                    <div className="bg-background dark:bg-darkBackground grid">
                        {selectedListing ? (
                            <div
                                key={selectedListing.id}
                                className={`w-full bg-background dark:bg-darkBackground`}
                            >
                                            ...
                                            >
                                                <div className="relative">
                                                    <Listbox.Button className="relative w-full cursor-default overflow-hidden rounded-lg bg-white dark:bg-darkForeground text-left shadow-sm focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-primary sm:text-sm">
                                                        ...
                                                        <span className="ml-8 text-3xl font-medium text-black dark:text-white">
                                                    ...
                                                    <RoundButton
                                                        {...{
                                                            ...
                                                            onClick: () =>
                                                                apiRequest(
                                                                    'PATCH',
                                                                    `/client/listings/${selectedListing.id}/visibility`,
                                                                    {
                                                                        visibility:
                                                                            true,
                                                                    }
                                                                    ...
                                                                    if (
                                                                        ...
                                                                        addNotification(
                                                                            {
                                                                                type: 'success',
                                                                                title: 'Listing unhid successfully',
                                                                                text: 'Refreshing your view...',
                                                                            }
                                                                        );
                                                                ...
                                                                apiRequest(
                                                                    ...
                                                                    if (
                                                                        ...
                                                                        addNotification(
                                                                            {
                                                                                type: 'success',
                                                                                title: 'Listing hid successfully',
                                                                                text: 'Refreshing your view...',
                                                                            }
                                                                        );
                                                                        window.location.reload();
                                ...
                                <div className="bg-background dark:bg-darkBackground pl-8 pr-8">
                                    <div className="bg-background dark:bg-darkBackground flex flex-row justify-between flex-wrap mx-auto">
                                            ...
                                            <div
                                                ...
                                                ${
                                                    selectedStatus === status && status === "hired"
                                                        ? `shadow-hired`
                                                        : ''
                                                } hover:${selectedStatus !== status ? "shadow-lg" : ""} bg-white dark:bg-darkForeground flex flex-col w-[176px] h-[187px] rounded-2xl p-4 justify-between items-center my-8`}
                                            >
                                            <div>
                                            {status === "bookmarked" ? 
    ...
    <path d="M22.4855 29.5807L15.8272 17.6659C15.0616 16.2959 14.6788 15.6109 14.7433 15.05C14.7996 14.5607 15.0595 14.1177 15.4592 13.8299C15.9173 13.5 16.702 13.5 18.2715 13.5H22.1824C22.7657 13.5 23.0573 13.5 23.3189 13.5842C23.5504 13.6588 23.7638 13.7807 23.9455 13.9423C24.1508 14.125 24.2988 14.3762 24.5949 14.8788L30.9992 25.75L37.4035 14.8788C37.6996 14.3762 37.8476 14.125 38.053 13.9423C38.2347 13.7807 38.4481 13.6588 38.6795 13.5842C38.9411 13.5 39.2327 13.5 39.816 13.5H43.727C45.2964 13.5 46.0811 13.5 46.5392 13.8299C46.9389 14.1177 47.1988 14.5607 47.2551 15.05C47.3196 15.6109 46.9368 16.2959 46.1712 17.6659L39.5129 29.5807M28.3742 34.5L30.9992 32.75V41.5M28.8117 41.5H33.1867M39.0426 29.0817C43.4848 33.5239 43.4848 40.7261 39.0426 45.1683C34.6004 49.6106 27.3981 49.6106 22.9559 45.1683C18.5137 40.7261 18.5137 33.5239 22.9559 29.0817C27.3981 24.6394 34.6003 24.6394 39.0426 29.0817Z" stroke="#56CCF2" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
                        ...
                        {showManage ? (
                            <ApplicationManagement
                                {...{
                                    applications:
                                        filteredApplications(selectedStatus),
...
const ClientHiringCenter: NextPage = () => {
    return (
        <>
            <Listings />
        </>
...
export default ClientHiringCenter;
